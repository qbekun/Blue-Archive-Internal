...
void get_back_buffer(IDXGISwapChain* swap_chain) {
	if (!device || !ctx || !swap_chain) return;
	...
	if (FAILED(device->CreateTexture2D(&desc, nullptr, &back_buffer_tex)) || !back_buffer_tex)
	{
		back_buffer->Release();
		return;
	}
...
void begin(const ImDrawList* d, const ImDrawCmd* cmd) {
	...
	if (!pshader) {
		if (FAILED(device->CreatePixelShader(pshaderd, sizeof(pshaderd), NULL, &pshader)))
			return;
	}

	auto bd = (ImGui_ImplDX11_Data*)ImGui::GetIO().BackendRendererUserData;
	if (!bd || !bd->pFontSampler)
		return;
