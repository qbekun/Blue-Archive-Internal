...
{
	// The background wallpaper needs D3DX11, which isn't available in the
	// injected context. The fullscreen wallpaper is a standalone-example
	// feature, so this is intentionally left empty for the game overlay.
}
...
bool ui::items::notify_checkbox(const char* label, bool* v)
{
	const bool changed = ImGui::Checkbox(label, v);
	if (changed)
	{
		const std::string msg = std::string(label) + (*v ? " enabled" : " disabled");
		ui::add_notification(*v ? "A" : "B", msg,
			*v ? ImVec4(0.56f, 0.93f, 0.56f, 1.0f) : ImVec4(0.99f, 0.60f, 0.60f, 1.0f));
	}
	return changed;
}
...
void ui::render_watermark()
{
	...
	// Build the info part from the enabled options: user  |  time  |  FPS
	std::string user_text;
	if (g_Options.bWatermarkUser && g_Options.WatermarkUser[0] != '\0')
		user_text += g_Options.WatermarkUser;

	if (g_Options.bWatermarkTime)
	{
		SYSTEMTIME st;
		GetLocalTime(&st);
		char time_buf[16];
	...
	if (g_Options.bWatermarkFPS)
	{
		if (!user_text.empty()) user_text += "  |  ";
		user_text += "FPS: " + std::to_string((int)ImGui::GetIO().Framerate);
	}

	const bool has_info = !user_text.empty();

	ImGui::PushFont(font.spacegrotesk_medium[1]);
	ImVec2 username_size = has_info ? ImGui::CalcTextSize(user_text.c_str()) : ImVec2(0, 0);
	ImGui::PopFont();
	...
	float total_width = watermark_size.x;
	if (has_info)
		total_width += spacing + separator_width + spacing + username_size.x;
	...
	// Menu name with the same text -> accent gradient used across the menu.
	ImGui::PushFont(font.montserrat_semibold[0]);
	{
		std::string name_str = watermark_text;
		int length = (int)name_str.size();
		ImVec2 cursor_pos = start_pos;
